...
async def ai_count_gallons(body: AICountRequest, user=Depends(get_current_user)):
    ...
    system = (
        ...
        "  | Suzuki Carry                    | 54 galon                                    |\n"
        "  | Daihatsu Granmax                | 52 galon                                    |\n"
        "  | Suzuki Mega Carry               | 48 galon                                    |\n"
        "  | Pickup kecil generik (default)  | 50 galon                                    |\n"
        "  | Truk engkel colt diesel         | 90 galon                                    |\n"
        "  | Truk fuso                       | 130 galon                                   |\n"
        "  \n"
        "  ATURAN PENERAPAN:\n"
        "  1. Identifikasi jenis mobil dari foto (lihat body, cabin, logo — Carry biasanya lebih boxy, Granmax lebih modern, Mega Carry lebih tinggi/besar).\n"
        "     Kalau tidak yakin, pakai default \"pickup generik = 50 galon\".\n"
        "  2. Amati apakah galon DITUMPUK di atas mobil:\n"
        "     a. Kalau HANYA 1 LAPIS (satu tumpuk saja, tinggi galon = 1 baris) → COUNT = kapasitas baku mobil itu.\n"
        "        Contoh: Granmax 1 lapis → count = 52.\n"
        "     b. Kalau 2 LAPIS (ditumpuk 2 tingkat) → COUNT = kapasitas_baku + hitung_galon_terlihat_di_LAPIS_ATAS.\n"
        "        Karena lapis bawah = full bak, jadi otomatis pakai kapasitas baku (jangan dihitung ulang).\n"
        "        Hanya perlu hitung galon di LAPIS ATAS yang terlihat menyembul di atas cabin/atap bak.\n"
        "        Contoh: Granmax 2 tumpuk, terlihat 18 galon di atas → count = 52 + 18 = 70.\n"
        "     c. Kalau 3 LAPIS → COUNT = kapasitas_baku + (kapasitas_baku × 0.7) + galon_di_lapis_teratas.\n"
        "        Lapis kedua kurang lebih 70% penuh, lapis atas biasanya lebih sedikit.\n"
        "     d. Kalau bak TIDAK penuh (ada rongga jelas) → estimasi persentase kepenuhan × kapasitas baku.\n"
        "        Contoh: Carry setengah penuh 1 lapis → count = 54 × 0.5 ≈ 27.\n"
        "  3. Reasoning WAJIB sebut jenis mobil + kapasitas baku + berapa lapis + galon lapis atas.\n"
        "     Contoh: \"Strategi C: Granmax 2 tumpuk. Bak bawah = 52 (kapasitas baku), lapis atas terlihat 15 galon. Total = 52+15 = 67.\"\n"
        "  \n"
        "  Confidence:\n"
        "  - HIGH kalau jelas 1 lapis full & jenis mobil terkonfirmasi.\n"
        "  - MEDIUM kalau 2 lapis (lapis atas bisa dihitung).\n"
        "  - LOW kalau ≥3 lapis atau mobil tidak dikenali.\n\n"
