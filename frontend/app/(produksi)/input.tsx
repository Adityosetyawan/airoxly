...
export default function ProduksiInput() {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      toast.show("Input produksi berhasil disimpan", "success");
      setForm((f) => ({
        ...f,
    ...
    <View style={{ flex: 1, backgroundColor: theme.color.surfaceSecondary }}>
      ...
      <ScrollView contentContainerStyle={styles.body}>
        <View style={styles.card}>
          ...
          <Row label="Shift">
            <View style={styles.shiftRow}>
              {(["pagi", "siang"] as const).map((s) => (
                <TouchableOpacity
                  key={s}
                  onPress={() => setF("shift", s)}
                  style={[styles.shiftBtn, form.shift === s && styles.shiftBtnOn]}
                >
          ...
          <NumField label="Sil / Seal Ganti" value={form.sil_ganti} onChange={(v) => setF("sil_ganti", v)} color="#EF4444" />
...
const styles = StyleSheet.create({
  ...
  numRow: { flexDirection: "row", alignItems: "center", gap: 8, marginTop: 2 },
